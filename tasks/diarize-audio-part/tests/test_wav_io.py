...
@pytest.mark.parametrize("frame_count", [15_985, 16_015])
def test_read_accepts_sub_millisecond_boundary_rounding(
...
def test_read_rejects_one_millisecond_boundary_difference(
    tmp_path: Path, frame_count: int
) -> None:
    source = tmp_path / "source.wav"
    write_source(source, np.zeros(frame_count, dtype="<i2"))
