...
class DiarizationPublisher:
    """Select the diarization contract for the shared task publisher."""

    def __init__(self, task_publisher: TaskPublisher) -> None:
        self._task_publisher = task_publisher

    @classmethod
    def create(cls, settings: EnvironmentSettings) -> DiarizationPublisher:
        return cls(
            TaskPublisher.create(
                client_name="voice-pipeline-split-raw-audio-publisher",
    ...
    def publish(self, audio_part_id: UUID) -> str:
        return self._task_publisher.publish(
            DIARIZE_AUDIO_PART,
            audio_part_id,
        )
