    ...
    def __call__(self, value):
        ...
        try:
            ...
            for slot, (speaker, audio) in enumerate(
                ...
                if units:
                    units = restore_punctuation(
                        units,
                        self.punctuation,
                        max_chars=self.policy.punctuation.max_chars,
                    )
                utterances = build_utterances(units, self.policy.utterance)
            ...
            try:
                ...
                logger.error("transcribe_chunk_zh.failure_persistence_failed")
            ...
            try:
                ...
                logger.error("transcribe_chunk_zh.workspace_cleanup_failed")
...
def register(app: Celery, handler):
    @app.task(
        name=TRANSCRIBE_CHUNK_ZH.name,
        queue=TRANSCRIBE_CHUNK_ZH.queue,
        bind=True,
        acks_late=True,
        reject_on_worker_lost=True,
        ignore_result=True,
    )
    def transcribe_chunk_zh(_task, value):
        return handler(value)

    return transcribe_chunk_zh
